...
config.icon = (function () {
  const canvas = new OffscreenCanvas(2 * 19, 2 * 19);
      ...
      app.tab.query.active(function (tab) {
        if (tab) {
          if (tab.url) {
            const values = [];
            const lines = [5, 15, 25];
            const blacklist = config.blacklist.domains;
            const whitelist = config.whitelist.domains;
            const hostname = (new URL(tab.url)).hostname;
            const status = config.addon.state === "OFF" ? "disable" : (blacklist.indexOf(hostname) !== -1 ? "error" : (whitelist.indexOf(hostname) !== -1 ? "disable" : "enable"));
            /*  */
            for (let i = 0; i < 3; i++) {
            ...
            app.storage.get({"selected": {"name": "Default"}}, async function (e) {
              const image = context.getImageData(0, 0, 2 * 19, 2 * 19);
              const title = "Audio Equalizer" + (status === "error" ? " :: Error" : (status === "disable" ? " :: Disabled" : " :: " + e.selected.name));
              /*  */
