...
app.on.message(function (request, sender, sendResponse) {
  ...
  const url = sender.tab && sender.tab.url ? sender.tab.url : (sender.url ? sender.url : '');
  /*  */
  if (request) {
    ...
    if (request.action === "app-icon-normal") {
      const hostname = url ? (new URL(url)).hostname : '';
      if (hostname) {
        const index = config.blacklist.domains.indexOf(hostname);
