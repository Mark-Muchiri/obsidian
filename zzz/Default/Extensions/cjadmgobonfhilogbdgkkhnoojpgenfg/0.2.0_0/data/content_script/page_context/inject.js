{
  var core = {
    ...
    "hostname": function (url) {
      ...
      const flag = (match !== null) && (match.length > 2) && (typeof match[2] === "string") && (match[2].length > 0);
    ...
    "equalizer": {
      ...
      "context": null,
      "gain": function () {
        const filter = core.equalizer.context.createGain();
        filter.channelCountMode = "explicit";
        filter.gain.value = 1;
        return filter;
      },
      "biquad": function (e) {
        const filter = core.equalizer.context.createBiquadFilter();
      ...
      "load": function () {
        ...
        for (let i = 0; i < core.equalizer.storage.eq.length; i++) {
          const node = core.equalizer.storage.eq[i];
          const filter = node.f ? core.equalizer.biquad(node) : core.equalizer.gain();
          if (filter) {
        ...
        if (!core.equalizer.context) core.equalizer.load();
          ...
          core.equalizer.targets.forEach(function (t, k) {
            if (t.getAttribute("equalizer-state") !== "attached") {
              const src = t.src ? t.src : t.currentSrc;
              if (src) {
                ...
                if (document.location.hostname !== core.hostname(src)) {
                  const crossorigin = t.getAttribute("crossorigin");
                  if (!crossorigin) {
                ...
                try {
                  ...
                  const target = core.equalizer.storage.state === "OFF" ? core.equalizer.context.destination : core.equalizer.filters[0];
                  core.equalizer.source.connect(target);
                  //
                  for (let i = 0; i < core.equalizer.filters.length; i++) {
                    const current = core.equalizer.filters[i];
                    const next = core.equalizer.filters[i + 1];
                    //
                    current.gain.value = core.equalizer.storage.eq[i].gain;
                    if (next) current.connect(next);
                  }
                  //
                  const last = core.equalizer.filters[core.equalizer.filters.length - 1];
