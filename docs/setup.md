...
<!-- <div align="center">
  <img src="../media/setup-banner-trial.webm" alt="Terminal Setup Animation" width="600" style="border-radius: 8px;"/>
  <br/>
...
1. [🚀 Fresh install — `setup.sh`](#1-fresh-install)
2. [🔗 Restore configs — `restore.sh`](#2-restore-configs)
3. [🔄 Sync changes — `sync.sh`](#3-sync-changes)
4. [➕ Add a new config](#4-add-a-new-config)
5. [🛠️ Troubleshooting](#5-troubleshooting)

---

<a id="1-fresh-install"></a>
...
>
> **This is expected** — the new shell must be actively running before subsequent stages can continue.
...
Edit `packages.conf` before running setup, or add packages to it at any time and re-run:
...
Every entry in `CONFIG_MAP` inside `restore.sh` maps a repo path to a system path:
...
> ```
>
> This removes all managed symlinks and moves the most recent backup back into its original place.

---

<a id="3-sync-changes"></a>
...
<details>
<summary><b>💡 Why no manual copy step is needed (click to expand)</b></summary>
<br>

Because `~/.zshrc` is a symlink pointing to `configs/zsh/.zshrc`, editing `~/.zshrc` edits the repo file directly. `git add -A` picks it up immediately.
...
Delete only that stage's line from the state file, then resume:
